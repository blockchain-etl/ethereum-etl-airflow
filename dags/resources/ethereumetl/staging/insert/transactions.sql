INSERT INTO $CHAIN.transactions
SELECT
    staged_transactions.created_time AS created_time,
    staged_transactions.hash AS hash,
    staged_transactions.nonce AS nonce,
    staged_transactions.block_hash AS block_hash,
    staged_transactions.block_number AS block_number,
    staged_transactions.transaction_index AS transaction_index,
    staged_transactions.from_address AS from_address,
    staged_transactions.to_address AS to_address,
    staged_transactions.value AS value,
    staged_transactions.gas AS gas,
    staged_transactions.gas_price AS gas_price,
    staged_transactions.input AS input,
    staged_transactions.receipt_cumulative_gas_used AS receipt_cumulative_gas_used,
    staged_transactions.receipt_gas_used AS receipt_gas_used,
    staged_transactions.receipt_contract_address AS receipt_contract_address,
    staged_transactions.receipt_root AS receipt_root,
    staged_transactions.receipt_status AS receipt_status,
    staged_transactions.block_timestamp AS block_timestamp,
    staged_transactions.block_date AS block_date
FROM
    $CHAIN.transactions_staged_$EXECUTION_DATE_NODASH AS staged_transactions;
