...
def scrape_contract_abis_command(execution_date, **kwargs):
    with TemporaryDirectory() as tempdir:
        copy_from_export_path(
            export_path("tokens", execution_date), os.path.join(tempdir, "tokens.csv")
        )

        set_max_field_size_limit()

        extract_csv_column.callback(
            input=os.path.join(tempdir, "tokens.csv"),
        ...
        with smart_open(os.path.join(tempdir, "contract_addresses.txt"), 'r') as addresses_file,\
                smart_open(os.path.join(tempdir, "token_abis.csv"), 'w') as output_file:
        ...
        copy_to_export_path(
            os.path.join(tempdir, "token_abis.csv"), export_path("token_abis", execution_date)
        )
